...
class ProspectCrud:
    async def create_prospect_from_csv(
        ...
        existing_prospect = db.query(Prospect).filter(Prospect.email == data[options["email_index"]]).all()
                ...
                tasks.append(
                    create_task(
                        self.create_prospect_from_csv(
                            self, db=db, user_id=user_id, file_id=id, data=row, options=options
                        )
                    )
                )
        """
